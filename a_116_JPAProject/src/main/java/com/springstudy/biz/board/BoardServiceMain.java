 ...
 * 4) find(Class<T> entityClass, Object primaryKey) :
 * 							SELECT ONE(1개 검색)
 * 5) createQuery(String qlString, Class<T> resultClass) :
 * 			SELECT LIST(JPQL에 해당하는 목록 검색)
 * */
...
public class BoardServiceMain {
	public static void main(String[] args) {
		...
		EntityManagerFactory emf = 
				Persistence.createEntityManagerFactory("a_116_JPAProject");
		...
		try {
			...
			// 테이블 목록 조회
			String jpql = "select b from Board b order" + "\r\n"
						+ "by b.seq desc";
